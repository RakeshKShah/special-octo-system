...
CASE_SUFFIX="$(date +%s)-$$"
EMAIL="validation-price-${CASE_SUFFIX}@example.com"
PASSWORD="Passw0rd!${CASE_SUFFIX}"
STORE_NAME="Price Shop ${CASE_SUFFIX}"
REGISTER_FILE="/tmp/validation_invalid_price_cents_register_${CASE_SUFFIX}.json"
RESPONSE_FILE="/tmp/validation_invalid_price_cents_${CASE_SUFFIX}.json"
USER_ID=""
TOKEN=""
...
# When — submit a product payload with invalid negative price_cents.
HTTP_STATUS="$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  -X POST "$BASE_URL/products" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer ${TOKEN}" \
  --data '{"title":"Product","description":"Test","category":"Misc","price_cents":-500,"stock_qty":1,"photos":[]}')"

# Then — response is 400 with a validation error payload.
[ "$HTTP_STATUS" = "400" ]
grep -F '"error":' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:validation_invalid_price_cents"
