...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO users (id, email, password_hash, role, status, created_at) VALUES ('$SELLER_USER_ID', 'nullbio-${CASE_SUFFIX}@example.com', 'hash', 'SELLER', 'ACTIVE', NOW());"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('$SELLER_ID', '$SELLER_USER_ID', 'New Store', NULL);"
