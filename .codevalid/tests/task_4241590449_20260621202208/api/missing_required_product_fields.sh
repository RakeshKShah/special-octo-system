...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H 'Content-Type: application/json' -H "Authorization: Bearer $TOKEN" -d "{\"description\":\"No title\",\"category\":\"ELECTRONICS\",\"price_cents\":1999,\"stock_qty\":5,\"photos\":[]}" "$BASE_URL/products")

# Then — HTTP/body assertions
[ "$HTTP_CODE" = "400" ]
grep -i 'title' "$RESPONSE_FILE" >/dev/null
