...
BUYER_EMAIL="buyer_pending_${CASE_SUFFIX}@example.com"
STORE_NAME="Pending Store ${CASE_SUFFIX}"
PRODUCT_TITLE="Pending Product ${CASE_SUFFIX}"
ORDER_ID="order-pending-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/invalid_order_status_rejected_${CASE_SUFFIX}.json"
SELLER_FILE="/tmp/invalid_order_status_rejected_seller_${CASE_SUFFIX}.json"
BUYER_FILE="/tmp/invalid_order_status_rejected_buyer_${CASE_SUFFIX}.json"
PRODUCT_FILE="/tmp/invalid_order_status_rejected_product_${CASE_SUFFIX}.json"
...
# Given — create active seller and buyer, create seller product, seed a PENDING order containing the seller item.
...
PRODUCT_HTTP_STATUS="$(curl -sS -o "$PRODUCT_FILE" -w '%{http_code}' -X POST "$BASE_URL/products" -H "Authorization: Bearer ${SELLER_TOKEN}" -H 'Content-Type: application/json' --data "{\"title\":\"${PRODUCT_TITLE}\",\"description\":\"desc\",\"category\":\"general\",\"price_cents\":1300,\"stock_qty\":4,\"photos\":[\"https://example.com/${CASE_SUFFIX}.jpg\"]}")"
...
VALUES ('${ORDER_ID}', '${BUYER_USER_ID}', 'PENDING', 13, NOW(), NOW());
INSERT INTO "OrderItem" (id, "orderId", "productId", qty, "sellerPayoutCents")
VALUES ('oi-pending-${CASE_SUFFIX}', '${ORDER_ID}', '${PRODUCT_ID}', 1, 1000);
SQL

# When — attempt to ship the PENDING order.
...
echo "CODEVALID_TEST_ASSERTION_OK:invalid_order_status_rejected"
