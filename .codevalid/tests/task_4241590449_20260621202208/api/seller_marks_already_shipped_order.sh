...
BUYER_EMAIL="buyer-shipped-${CASE_SUFFIX}@example.com"
SELLER_EMAIL="seller-shipped-${CASE_SUFFIX}@example.com"
PRODUCT_ID="prod-shipped-${CASE_SUFFIX}"
ORDER_ITEM_ID="order-item-shipped-${CASE_SUFFIX}"
SELLER_TOKEN=""
SELLER_USER_ID=""
SELLER_PROFILE_ID=""
BUYER_USER_ID=""
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Product\" (id, \"sellerId\", title, description, category, \"priceCents\", \"stockQty\", photos, status, visible, \"createdAt\", \"updatedAt\") VALUES ('$PRODUCT_ID', '$SELLER_PROFILE_ID', 'Product $CASE_SUFFIX', 'Seeded product', 'GENERAL', 2500, 5, '[]', 'ACTIVE', true, NOW(), NOW())" >/dev/null
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Order\" (id, \"buyerId\", status, \"createdAt\", \"updatedAt\") VALUES ('$ORDER_ID', '$BUYER_USER_ID', 'SHIPPED', NOW(), NOW())" >/dev/null
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"OrderItem\" (id, \"orderId\", \"productId\", qty, \"sellerPayoutCents\") VALUES ('$ORDER_ITEM_ID', '$ORDER_ID', '$PRODUCT_ID', 1, 2000)" >/dev/null
