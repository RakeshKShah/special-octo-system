...
CASE_SUFFIX="$(date +%s)-$$"
SELLER_EMAIL="seller_shipped_${CASE_SUFFIX}@example.com"
BUYER_EMAIL="buyer_shipped_${CASE_SUFFIX}@example.com"
STORE_NAME="Shipped Store ${CASE_SUFFIX}"
PRODUCT_TITLE="Shipped Product ${CASE_SUFFIX}"
ORDER_ID="order-shipped-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/seller_marks_already_shipped_order_${CASE_SUFFIX}.json"
SELLER_FILE="/tmp/seller_marks_already_shipped_order_seller_${CASE_SUFFIX}.json"
BUYER_FILE="/tmp/seller_marks_already_shipped_order_buyer_${CASE_SUFFIX}.json"
PRODUCT_FILE="/tmp/seller_marks_already_shipped_order_product_${CASE_SUFFIX}.json"
...
# Given — create active seller and buyer, create seller product, seed an order already SHIPPED containing seller item.
...
PRODUCT_HTTP_STATUS="$(curl -sS -o "$PRODUCT_FILE" -w '%{http_code}' -X POST "$BASE_URL/products" -H "Authorization: Bearer ${SELLER_TOKEN}" -H 'Content-Type: application/json' --data "{\"title\":\"${PRODUCT_TITLE}\",\"description\":\"desc\",\"category\":\"general\",\"price_cents\":2100,\"stock_qty\":5,\"photos\":[\"https://example.com/${CASE_SUFFIX}.jpg\"]}")"
