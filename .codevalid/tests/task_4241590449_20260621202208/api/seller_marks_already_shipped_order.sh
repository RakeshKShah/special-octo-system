...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible, created_at) VALUES ('$PRODUCT_ID', '$SELLER_PROFILE_ID', 'Product $CASE_SUFFIX', 'Seeded product', 'GENERAL', 2500, 5, '[]', 'ACTIVE', true, NOW())" >/dev/null
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO orders (id, buyer_id, status, created_at) VALUES ('$ORDER_ID', '$BUYER_USER_ID', 'SHIPPED', NOW())" >/dev/null
