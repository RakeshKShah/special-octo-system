...
BUYER_USER_ID=""
cleanup() {
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM order_items WHERE id IN ('$ORDER_ITEM1_ID', '$ORDER_ITEM2_ID')" >/dev/null 2>&1 || true
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM orders WHERE id = '$ORDER_ID'" >/dev/null 2>&1 || true
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM products WHERE id IN ('$PRODUCT1_ID', '$PRODUCT2_ID')" >/dev/null 2>&1 || true
...
HTTP_CODE=$(curl -sS -o "$REGISTER_RESPONSE_FILE" -w '%{http_code}' -X POST -H 'Content-Type: application/json' -d "{\"email\":\"$SELLER1_EMAIL\",\"password\":\"Password123!\",\"role\":\"SELLER\",store_name:\"Store 1 $CASE_SUFFIX\",\"bio\":\"Bio\"}" "$BASE_URL/auth/register")
[ "$HTTP_CODE" = "201" ]
...
HTTP_CODE=$(curl -sS -o "$REGISTER_RESPONSE_FILE" -w '%{http_code}' -X POST -H 'Content-Type: application/json' -d "{\"email\":\"$SELLER2_EMAIL\",\"password\":\"Password123!\",\"role\":\"SELLER\",store_name:\"Store 2 $CASE_SUFFIX\",\"bio\":\"Bio\"}" "$BASE_URL/auth/register")
[ "$HTTP_CODE" = "201" ]
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible, created_at) VALUES ('$PRODUCT1_ID', '$SELLER1_PROFILE_ID', 'Product 1 $CASE_SUFFIX', 'Seeded product', 'GENERAL', 2500, 5, '[]', 'ACTIVE', true, NOW())" >/dev/null
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible, created_at) VALUES ('$PRODUCT2_ID', '$SELLER2_PROFILE_ID', 'Product 2 $CASE_SUFFIX', 'Seeded product', 'GENERAL', 2600, 5, '[]', 'ACTIVE', true, NOW())" >/dev/null
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO order_items (id, order_id, product_id, seller_id, qty, price_at_purchase, seller_payout_cents) VALUES ('$ORDER_ITEM1_ID', '$ORDER_ID', '$PRODUCT1_ID', '$SELLER1_PROFILE_ID', 1, 2500, 2000)" >/dev/null
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO order_items (id, order_id, product_id, seller_id, qty, price_at_purchase, seller_payout_cents) VALUES ('$ORDER_ITEM2_ID', '$ORDER_ID', '$PRODUCT2_ID', '$SELLER2_PROFILE_ID', 1, 2600, 2100)" >/dev/null
