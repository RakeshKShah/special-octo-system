...
CREATE_CODE=$(curl -sS -o "$CREATE_RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"title\":\"Removed Product ${CASE_SUFFIX}\",\"description\":\"Original description\",\"category\":\"tools\",\"price_cents\":1999,\"stock_qty\":10,\"photos\":[\"https://example.com/original-${CASE_SUFFIX}.jpg\"]}" "$BASE_URL/products")
...
HTTP_CODE=$(curl -sS -o "$UPDATE_RESPONSE_FILE" -w '%{http_code}' -X PUT -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"title\":\"Attempt Restore\"}" "$BASE_URL/products/$PRODUCT_ID")
...
printf '%s\n' 'CODEVALID_TEST_ASSERTION_OK:update_product_removed_status'
