...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"order_item_id\":\"$ORDER_ITEM_ID\",\"rating\":3,\"body\":\"OK\"}" "$BASE_URL/reviews")

# Then — HTTP/body assertions
[ "$HTTP_CODE" = "201" ]
grep -F "$ORDER_ITEM_ID" "$RESPONSE_FILE" >/dev/null
grep -F '"body":"OK"' "$RESPONSE_FILE" >/dev/null || grep -F 'OK' "$RESPONSE_FILE" >/dev/null

# Cleanup — undo Given side effects
# handled by trap

echo 'CODEVALID_TEST_ASSERTION_OK:review_with_minimal_body_accepted'
