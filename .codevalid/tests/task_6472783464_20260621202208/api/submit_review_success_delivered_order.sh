...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"order_item_id\":\"$ORDER_ITEM_ID\",\"rating\":5,\"body\":\"Great product, fast delivery!\"}" "$BASE_URL/reviews")

# Then — HTTP/body assertions
[ "$HTTP_CODE" = "201" ]
grep -F '"orderItemId"' "$RESPONSE_FILE" >/dev/null
grep -F "$ORDER_ITEM_ID" "$RESPONSE_FILE" >/dev/null
grep -F "$PRODUCT_ID" "$RESPONSE_FILE" >/dev/null
grep -F "$BUYER_ID" "$RESPONSE_FILE" >/dev/null
grep -F '"rating":5' "$RESPONSE_FILE" >/dev/null
grep -F 'Great product, fast delivery!' "$RESPONSE_FILE" >/dev/null

# Cleanup — undo Given side effects
# handled by trap

echo 'CODEVALID_TEST_ASSERTION_OK:submit_review_success_delivered_order'
