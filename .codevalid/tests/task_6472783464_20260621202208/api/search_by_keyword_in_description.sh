...
if [ -n "$BUYER_AUTH_HEADER" ]; then
  HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X GET -H "$BUYER_AUTH_HEADER" "$BASE_URL/products?keyword=CERAMIC")
else
  HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X GET "$BASE_URL/products?keyword=CERAMIC")
fi
