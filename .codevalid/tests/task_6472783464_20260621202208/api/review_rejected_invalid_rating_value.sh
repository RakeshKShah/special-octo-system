...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"order_item_id\":\"$ORDER_ITEM_ID\",\"rating\":6,\"body\":\"Rating too high\"}" "$BASE_URL/reviews")
...
echo 'CODEVALID_TEST_ASSERTION_OK:review_rejected_invalid_rating_value'
