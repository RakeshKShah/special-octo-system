...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Order\" (id, \"buyerId\", \"totalCents\", \"platformFeeCents\", status) VALUES ('$ORDER_ID', '$BUYER_ID', 1200, 120, 'SHIPPED');" >/dev/null
...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"order_item_id\":\"$ORDER_ITEM_ID\",\"rating\":4,\"body\":\"Looking forward to receiving it\"}" "$BASE_URL/reviews")

# Then — HTTP/body assertions
[ "$HTTP_CODE" = "400" ]
grep -F 'Reviews are only allowed after the order has been delivered' "$RESPONSE_FILE" >/dev/null

# Cleanup — undo Given side effects
# handled by trap

echo 'CODEVALID_TEST_ASSERTION_OK:review_rejected_order_not_delivered'
