...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"SellerProfile\" (id, \"userId\", \"storeName\") VALUES ('$SELLER_ID', '$SELLER_USER_ID', 'Removed Store') ON CONFLICT (id) DO NOTHING;"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Product\" (id, \"sellerId\", title, description, category, \"priceCents\", \"stockQty\", status, visible) VALUES ('$PRODUCT_ID', '$SELLER_ID', 'Removed ${CASE_SUFFIX}', 'Removed product', 'crafts', 1000, 5, 'REMOVED', true);"
...
echo 'CODEVALID_TEST_ASSERTION_OK:checkout_product_removed_status'
