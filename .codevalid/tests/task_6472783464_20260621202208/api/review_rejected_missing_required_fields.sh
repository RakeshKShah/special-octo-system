...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d '{"rating":3}' "$BASE_URL/reviews")
...
echo 'CODEVALID_TEST_ASSERTION_OK:review_rejected_missing_required_fields'
