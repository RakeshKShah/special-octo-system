...
PRODUCT_ID="prod-last-unit-${CASE_SUFFIX}"
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"SellerProfile\" (id, \"userId\", \"storeName\") VALUES ('$SELLER_ID', '$SELLER_USER_ID', 'Last Unit Store') ON CONFLICT (id) DO NOTHING;"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Product\" (id, \"sellerId\", title, description, category, \"priceCents\", \"stockQty\", status, visible) VALUES ('$PRODUCT_ID', '$SELLER_ID', 'Last Unit ${CASE_SUFFIX}', 'Final inventory', 'crafts', 5000, 2, 'ACTIVE', true);"
...
PRODUCT_STATUS="$(psql "$DATABASE_URL" -tAc "SELECT status FROM \"Product\" WHERE id = '$PRODUCT_ID';")"
[ "$STOCK_QTY" = "0" ]
[ "$PRODUCT_STATUS" = "SOLD_OUT" ]

# Cleanup — undo Given side effects

echo 'CODEVALID_TEST_ASSERTION_OK:checkout_stock_decrement_and_status_sync'
