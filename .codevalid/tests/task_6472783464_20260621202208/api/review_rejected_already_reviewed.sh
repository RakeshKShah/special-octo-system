...
cleanup() {
  ...
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM \"Review\" WHERE id = '$REVIEW_ID' OR \"orderItemId\" = '$ORDER_ITEM_ID';" >/dev/null 2>&1 || true
  ...
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM \"User\" WHERE id IN ('$BUYER_ID', '$SELLER_USER_ID');" >/dev/null 2>&1 || true
}
trap cleanup EXIT

# Given — bring the system to the required state
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"User\" (id, email, password, role) VALUES ('$BUYER_ID', 'buyer-$CASE_SUFFIX@example.com', 'password123', 'BUYER'), ('$SELLER_USER_ID', 'seller-$CASE_SUFFIX@example.com', 'password123', 'SELLER');" >/dev/null
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Order\" (id, \"buyerId\", \"totalCents\", \"platformFeeCents\", status) VALUES ('$ORDER_ID', '$BUYER_ID', 1200, 120, 'DELIVERED');" >/dev/null
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO \"Review\" (id, \"orderItemId\", \"productId\", \"buyerId\", rating, body) VALUES ('$REVIEW_ID', '$ORDER_ITEM_ID', '$PRODUCT_ID', '$BUYER_ID', 4, 'Existing review');" >/dev/null
...
HTTP_CODE=$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\"order_item_id\":\"$ORDER_ITEM_ID\",\"rating\":5,\"body\":\"Trying to review again\"}" "$BASE_URL/reviews")

# Then — HTTP/body assertions
[ "$HTTP_CODE" = "400" ]
grep -F 'Already reviewed' "$RESPONSE_FILE" >/dev/null

# Cleanup — undo Given side effects
# handled by trap

echo 'CODEVALID_TEST_ASSERTION_OK:review_rejected_already_reviewed'
